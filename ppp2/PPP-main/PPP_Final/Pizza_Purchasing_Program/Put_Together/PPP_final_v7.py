...
while order_again == "yes":
    ...
    place_order = string_check(f"Would you like to place or cancel your order? ", order_ans, 1)

    if place_order == "cancel":
        # double check that the user wants to cancel their order and cancel it
        cancel = string_check("Are you sure you want to cancel your order? ")
        ...
        if cancel == "yes":
            ...
            if collect == "delivery":
                pay_method = string_check(
                    f"Including the delivery fee, your order comes to a total cost of ${price:.2f}. "
                    f"Will you be paying with cash or credit? ",
                    payment_ans, 2)
            else:
                pay_method = string_check(
                    f"In total, your order costs ${price:.2f}. Will you be paying with cash or credit? ",
                    payment_ans, 2)
        ...
        if collect == "delivery":
            pay_method = string_check(f"Including the delivery fee, your order comes to a total cost of ${price:.2f}. "
                                      f"Will you be paying with cash or credit? ",
                                      payment_ans, 2)
        else:
            pay_method = string_check(
